...
export const MicSelectorItem = ({ onClick, ...props }: MicSelectorItemProps) => {
  const { onValueChange, onOpenChange } = useContext(MicSelectorContext)

  const handleClick: typeof onClick = useCallback(
    (event) => {
      const value = (event.currentTarget as HTMLElement).dataset.value ?? props.value?.toString()
      if (value) {
        onValueChange?.(value)
      }
      onOpenChange?.(false)
    ...
    [onValueChange, onOpenChange, onClick, props.value],
  )

  return <CommandItem onClick={handleClick} {...props} />
}
